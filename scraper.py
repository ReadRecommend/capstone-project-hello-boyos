...
def strip_book(url):
    ...
    isbn_element = soup.find(itemprop="isbn")
    if isbn_element:
    ...
    if genres:
        book["genres"] = [
            genre.text.strip()
            for genre in soup.find_all(class_="actionLinkLite bookPageGenreLink")
        ]

    language_element = soup.find(itemprop="inLanguage")
    if language_element:
    ...
    reviews_element = soup.find(itemprop="reviewCount")
    if reviews_element:
        book["n_reviews"] = int(
